...
// I.e., if key has not changed DO NOT CALL start or stop
  ...
  while(1){
    while(1){
        ...
        last&=0xF;
       //Clock_Delay(800000); // 10ms, to debounce switch
        if(last!=0&&val==0) break;
        val = last;
    }
    val = last;
    if((val&1)!=0){period = 324017;}
    if((val&2)!=0){period = 257152;}
    if((val&4)!=0){period = 216216;}
    if((val&8)!=0){period = 192631;}
    Sound_Start(period);
    }
}
...
// Run main5 with Lab5Grader(3);   // Grgpader
...
uint32_t ampl[] = {16,17,19,20,22,23,25,26,
...
uint32_t DACind = 0;
void Sound_Start(uint32_t period){
  uint32_t val = period/32;
   SysTick->CTRL = 0;           // 1) disable SysTick during setup
  SysTick->LOAD = 0;           // 2) stops
  uint32_t*ptr = &SysTick->LOAD;
...
void SysTick_Handler(void){
    ...
    DACind++;
    DACind%=32;
    DAC5_Out(ampl[DACind]);
    Debug_Dump2(GPIOB->DOUT31_0);
