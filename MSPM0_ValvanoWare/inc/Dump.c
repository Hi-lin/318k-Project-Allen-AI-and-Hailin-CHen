...
// This function should also initialize Timer G12, call TimerG12_Init.
 TimerG12_Init();
DebugCnt = 0; // 0 to MAXBUF (0 is empty, MAXBUF is full)
}
...
//    if zero, the signal is considered low.  
  ...
  for(int i = 0; i<DebugCnt; i++){
    if((mask&DataBuffer[i])==0){
      hasdown = true;
    }
    else{
      if(hasdown){
        if(lastup!=-1){
          tperiod-=TimeBuffer[i];
          tperiod+=lastup;
          periodcnt++;
        }
        lastup = TimeBuffer[i];
  ...
  return tperiod/periodcnt; // average period
}
...
int sum = 0;
  ...
  for(int i = 0; i<DebugCnt; i++){
    if((mask&DataBuffer[i])==0){
      ...
      if(!hasup){
        firstup = TimeBuffer[i];
      }
      hasup = true;
    }
  }
  return tduty/dutycnt; // average duty cycle in percent
}

// Lab2 specific debugging code
uint32_t Theperiod, TheDuty;
void Dump(void){
  ...
  uint32_t result = Debug_Dump2(data);       // calls your Lab3 function
